...
public class SimulationForTrainDiffusion {
	int NUM_COR = 10;
	int NN = 3;
	int Num = 10;
	int T;
	...
	public static void main(String[] args) {
		...
		SimulationForTrainDiffusion pds = new SimulationForTrainDiffusion();
	...
	void process(double alphad, double alphain, double alphak, double alphaout, double cc, double deltat, double deltax,
			...
			int tmax, int tn, int ts, double vk, double vsigma, THR expect[]) {
		this.AlphaD = alphad;
	...
	double[] getOutputCor(int index) {
		double ret[] = new double[ltc2.get(index).size()];
		for (int i = 0; i < ltc2.get(index).size() - 1; i++)
			ret[i] = ltc2.get(index).get(i).calCorrelation();
		ret[ret.length - 1] = ret[ret.length - 2];
		return ret;
	}

	double[] getOutputCorPos() {
		double ret[] = new double[cp.length];
